...
export default async function handler(req, res) {
  ...
  const segredoEsperado = process.env.WELCOME_WEBHOOK_SECRET;
  if (!segredoEsperado) {
    console.error('[send-welcome-email] WELCOME_WEBHOOK_SECRET ausente no ambiente');
    return res.status(200).end();
  }
  if (req.headers['x-prisma-webhook-secret'] !== segredoEsperado) {
